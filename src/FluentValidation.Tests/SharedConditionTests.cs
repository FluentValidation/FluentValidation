...
public class SharedConditionTests {
	...
	class SharedCollectionConditionValidator : AbstractValidator<Person> {
		public SharedCollectionConditionValidator() {
			...
			When((x) => x.Id > 0,
				() => {
					RuleForEach(x => x.NickNames).NotEmpty();
				}
			);
		}
	}
	...
	[Fact]
	public async Task shared_async_when_not_applied_to_grouped_collection_rules_when_initial_predicate_is_false() {
		var validator = new SharedAsyncCollectionConditionValidator();
		var person = new Person(); // fails the shared When predicate
	...
	[Fact]
	public void Shared_When_is_applied_to_grouped_collection_rules_when_initial_predicate_is_true() {
		var validator = new SharedCollectionConditionValidator();
	...
	[Fact]
	public async Task Shared_async_When_is_applied_to_grouped_collection_rules_when_initial_predicate_is_true() {
		var validator = new SharedAsyncCollectionConditionValidator();
		var person = new Person() {
			Id = 4, // triggers the shared When predicate
			NickNames = new string[] { null },
	...
	[Fact]
	public void Shared_When_is_applied_to_grouped_rules_collection_when_initial_predicate_is_true_and_all_individual_rules_are_satisfied() {
		var validator = new SharedCollectionConditionValidator();
		var person = new Person() {
			Id = 4, // triggers the shared When predicate
			NickNames = new[] { "Foo"},
	...
	[Fact]
	public async Task Shared_async_When_is_applied_to_grouped_rules_collection_when_initial_predicate_is_true_and_all_individual_rules_are_satisfied() {
		var validator = new SharedAsyncCollectionConditionValidator();
		var person = new Person() {
			Id = 4, // triggers the shared When predicate
			NickNames = new[] { "foo" }
	...
	[Fact]
	public void Throws_when_shared_async_condition_invoked_synchronously() {
